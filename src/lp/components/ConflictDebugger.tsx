...
export function ConflictDebugger({ open, onOpenChange }: { open: boolean; onOpenChange: (o: boolean) => void }) {
  ...
  const conflicts = useMemo<Conflict[]>(() => {
    ...
    for (const i of issues) {
      const owner = i.scope === "group" || (i.scope === "perm" && groups.find(g => g.id === i.ownerId))
        ? groups.find(g => g.id === i.ownerId)
        : users.find(u => u.id === i.ownerId);
      if (!owner) continue;
      const ownerType: "group" | "user" = "permissions" in owner && "weight" in owner ? "group" : "user";
      const node = i.permId ? (owner as any).permissions.find((p: any) => p.id === i.permId)?.node || "?" : "—";
      const kind: Conflict["kind"] =
        /circular/i.test(i.message) ? "circular" :
        /duplicate/i.test(i.message) ? "duplicate" :
      ...
      const directNodes = new Set(g.permissions.map(p => p.node));
        ...
        if (directNodes.has(node)) {
          ...
          if (direct.value !== info.value && info.from !== g.name) {
            out.push({
              kind: "override", level: "info", ownerType: "group", ownerId: g.id, ownerName: g.name, node,
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-4xl h-[78vh] bg-panel border-border p-0 overflow-hidden flex flex-col">
        <DialogHeader className="px-5 py-3 border-b border-border bg-gradient-to-r from-titlebar via-panel to-titlebar relative">
          <div className="absolute inset-0 opacity-50 pointer-events-none" style={{ background: "var(--gradient-glint)" }} />
        ...
        <div className="flex-1 overflow-y-auto p-3 space-y-1.5">
          <AnimatePresence>
            {filtered.length === 0 && (
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="h-full grid place-items-center text-center py-12">
                <div>
                  ...
                  <div className="text-xs text-muted-foreground mt-1">Your permission tree is clean and consistent.</div>
                </div>
              </motion.div>
            )}
            {filtered.map((c, idx) => (
              <motion.div key={idx} initial={{ opacity: 0, x: -8 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0 }}
                ...
                <div className={`shrink-0 mt-0.5 w-7 h-7 rounded-md grid place-items-center ${
                  c.level === "error" ? "bg-destructive/15 text-destructive" :
                  c.level === "warning" ? "bg-warning/15 text-warning" :
                  "bg-info/15 text-info"
                }`}>
                  {c.kind === "circular" ? <GitBranch className="w-3.5 h-3.5" /> :
                   c.kind === "wildcard-shadow" ? <Layers className="w-3.5 h-3.5" /> :
                ...
                <button onClick={() => { setSelection({ type: c.ownerType, id: c.ownerId } as any); onOpenChange(false); }} className="flex-1 min-w-0 text-left">
                  <div className="flex items-center gap-2">
                    ...
                    {c.node !== "—" && <code className="text-[10px] font-mono px-1.5 py-0.5 rounded bg-input/60 text-primary truncate">{c.node}</code>}
                  </div>
