  ...
  return (
    <motion.div
      ...
      className={`relative px-4 py-3 rounded-xl border-2 cursor-pointer min-w-[200px] backdrop-blur-md transition-all ${
        selected
          ? "border-primary glow-neon-lg animate-neon-pulse"
    ...
    >
      ...
      <div className="flex items-center gap-2.5">
        <div
          className="w-9 h-9 rounded-lg grid place-items-center font-bold text-sm relative shrink-0"
          style={{
            background: `linear-gradient(135deg, ${g.color}, ${g.color}80)`,
            boxShadow: `0 0 16px ${g.color}80`,
          }}
        >
          {isOwner ? (
      ...
      {g.prefix && (
        <div className="mt-1.5 text-[10px] font-mono text-warning/90 truncate">
          prefix: {g.prefix}
        </div>
      )}
...
function UserNode({ data, selected }: any) {
  return (
    <motion.div
      ...
      style={{
        background:
          "linear-gradient(135deg, oklch(0.78 0.16 200 / 0.25), oklch(0.18 0.03 150 / 0.9))",
      }}
    >
      <Handle type="target" position={Position.Top} />
      <div
        ...
        style={{
          background: "linear-gradient(135deg, oklch(0.78 0.16 200), oklch(0.65 0.18 220))",
        }}
      >
    ...
    .forEach(([d, gs]) => {
      ...
      sorted.forEach((g, i) => {
        ...
        nodes.push({
          id: g.id,
          type: "groupNode",
          position: { x: i * COL_W - totalW / 2 + COL_W / 2, y: d * ROW_H },
...
function FamilyTreeInner() {
  ...
  const built = useMemo(
    () => layout(groups, users, onSelectGroup, onSelectUser),
    [groups, users, onSelectGroup, onSelectUser],
  );
  ...
  // selection sync
  const selectedId =
    selection?.type === "group" || selection?.type === "user" ? selection.id : undefined;
  ...
  const onConnect = useCallback(
    (c: Connection) => {
      ...
      if (targetIsUser) {
        ...
        if (u) {
          if (u.groups.includes(c.source)) {
            toast.info(`${u.username} already in group`);
            return;
          }
      ...
      toast.success("Inheritance link created", {
        description: "Source is now a parent of the target",
      });
    },
  ...
  return (
    <div className="absolute inset-0 grid-bg">
      ...
      >
        ...
        <MiniMap
          nodeColor={(n) =>
            n.type === "userNode"
              ? "oklch(0.78 0.16 200)"
              : (n.data as any)?.group?.color || "oklch(0.85 0.25 145)"
          }
          maskColor="oklch(0.10 0.02 150 / 0.7)"
        />
      </ReactFlow>
      <div className="absolute top-3 left-3 px-3 py-1.5 rounded-md glass border border-border text-[10px] font-mono text-muted-foreground pointer-events-none">
        <span className="text-primary">⬢</span> drag handles between nodes to inherit · click to
        inspect
      </div>
    </div>
  );
