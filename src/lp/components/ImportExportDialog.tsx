...
const FORMATS: { id: Format; name: string; desc: string; icon: any; ext: string; mime: string }[] = [
  { id: "vt-json", name: "Visual Tree JSON", desc: "Save & restore the full project (recommended)", icon: FileJson, ext: "vt.json", mime: "application/json" },
  { id: "lp-json", name: "LuckPerms JSON", desc: "Drop into LuckPerms /lp import", icon: FileJson, ext: "lp.json", mime: "application/json" },
  { id: "yaml", name: "YAML", desc: "Human-readable config", icon: FileCode, ext: "yml", mime: "text/yaml" },
  { id: "commands", name: "/lp Commands", desc: "Paste into your server console", icon: Terminal, ext: "txt", mime: "text/plain" },
];

export function ImportExportDialog({ open, onOpenChange, mode }: { open: boolean; onOpenChange: (o: boolean) => void; mode: "import" | "export" }) {
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-3xl bg-panel border-border p-0 overflow-hidden">
        <DialogHeader className="px-5 py-3 border-b border-border">
          <DialogTitle className="flex items-center gap-2">
            {mode === "export" ? <Download className="w-4 h-4 text-primary" /> : <Upload className="w-4 h-4 text-primary" />}
...
function ExportPanel() {
  ...
  const text = (() => {
    switch (format) {
      case "vt-json": return exportJSON(groups, users);
      case "lp-json": return exportLuckPermsJSON(groups, users);
      case "yaml": return exportYAML(groups, users);
      case "commands": return exportCommands(groups, users);
    }
  })();
  ...
  const copy = async () => { await navigator.clipboard.writeText(text); toast.success("Copied to clipboard"); };

  return (
    <div className="flex flex-col">
      <div className="grid grid-cols-4 gap-2 p-4 border-b border-border">
        {FORMATS.map(f => (
          <motion.button key={f.id} whileHover={{ y: -2 }} whileTap={{ scale: 0.97 }}
            onClick={() => setFormat(f.id)}
            className={`text-left p-3 rounded-lg border transition glint ${
              format === f.id ? "border-primary/60 bg-primary/10 glow-green" : "border-border bg-card/40 hover:border-primary/30"
            }`}>
      ...
      <div className="px-4 pt-3 pb-2 flex items-center gap-2">
        ...
        <span className="text-[10px] font-mono text-muted-foreground">· {text.split("\n").length} lines · {(text.length/1024).toFixed(1)} KB</span>
        <div className="ml-auto flex items-center gap-2">
          <button onClick={copy} className="flex items-center gap-1.5 px-3 py-1.5 rounded-md text-xs border border-border hover:border-primary/40 hover:bg-accent transition">
            <Copy className="w-3 h-3" /> Copy
          </button>
          <button onClick={download} className="glint flex items-center gap-1.5 px-3 py-1.5 rounded-md text-xs bg-primary text-primary-foreground glow-green hover:glow-green-soft transition font-semibold">
...
function ImportPanel({ onClose }: { onClose: () => void }) {
  ...
  const doImport = () => {
    if (!text.trim()) { toast.error("Nothing to import"); return; }
    try {
  ...
  return (
    <div className="p-4 space-y-4">
      <div>
        <div className="text-[10px] uppercase tracking-wider text-muted-foreground font-mono mb-2">templates</div>
        <div className="grid grid-cols-3 gap-2">
          {TEMPLATES.map(t => (
            <motion.button key={t.id} whileHover={{ y: -2 }} whileTap={{ scale: 0.97 }}
              ...
              <div className="mt-2 text-[10px] font-mono text-primary">{t.groups.length} groups →</div>
            </motion.button>
          ))}
        </div>
      </div>
      ...
      <div>
        <div className="flex items-center justify-between mb-2">
          <div className="text-[10px] uppercase tracking-wider text-muted-foreground font-mono">paste or upload</div>
            ...
            {(["merge", "replace"] as const).map(m => (
              <button key={m} onClick={() => setMergeMode(m)}
                className={`text-[10px] uppercase tracking-wider font-semibold px-2 py-1 rounded transition ${
                  mergeMode === m ? "bg-primary text-primary-foreground" : "text-muted-foreground hover:text-foreground"
                }`}>
                {m}
              </button>
            ))}
