...
export function Inspector() {
  const { selection, groups, users, updateGroup, updateUser, updatePermission, deletePermission, setSelection } = useStore();
  ...
  if (!selection) content = <Empty />;
    ...
    content = g ? (
      <div className="space-y-3">
        <NameField label="Group name" value={g.name} onChange={(v) => updateGroup(g.id, { name: v })} />
        <Block title="Display">
          <Field label="Prefix"><LabeledInput value={g.prefix || ""} onChange={(v) => updateGroup(g.id, { prefix: v })} placeholder="&6[VIP] " mono /></Field>
          <Field label="Suffix"><LabeledInput value={g.suffix || ""} onChange={(v) => updateGroup(g.id, { suffix: v })} mono /></Field>
          <Field label="Color">
            <div className="flex items-center gap-2">
              <input type="color" value={g.color || "#94a3b8"} onChange={(e) => updateGroup(g.id, { color: e.target.value })}
        ...
        <Block title="Hierarchy">
          <Field label={`Weight: ${g.weight}`}>
            <input type="range" min="0" max="1000" step="10" value={g.weight}
              onChange={(e) => updateGroup(g.id, { weight: Number(e.target.value) })}
              className="w-full accent-primary" />
            <div className="flex justify-between text-[9px] font-mono text-muted-foreground mt-0.5"><span>guest</span><span>owner</span></div>
          </Field>
          <Field label="Inherits from">
            <div className="flex flex-wrap gap-1.5">
              {groups.filter(x => x.id !== g.id).map(p => {
                const checked = g.parents.includes(p.id);
                return (
                  <button key={p.id} onClick={() => updateGroup(g.id, { parents: checked ? g.parents.filter(x => x !== p.id) : [...g.parents, p.id] })}
                    className={`flex items-center gap-1 px-2 py-1 rounded-md text-[11px] border transition ${
                      checked ? "bg-primary/15 border-primary/50 text-primary glow-neon" : "bg-card border-border text-muted-foreground hover:border-primary/40"
                    }`}>
    ...
    content = u ? (
      <div className="space-y-3">
        <NameField label="Username" value={u.username} onChange={(v) => updateUser(u.id, { username: v })} />
        <Block title="Identity">
          <Field label="UUID"><code className="block text-[10px] font-mono text-muted-foreground p-2 bg-input/40 rounded border border-border break-all">{u.uuid}</code></Field>
        </Block>
        <Block title="Member of">
          <div className="flex flex-wrap gap-1.5">
            {groups.map(g => {
              ...
              return (
                <button key={g.id} onClick={() => updateUser(u.id, { groups: checked ? u.groups.filter(x => x !== g.id) : [...u.groups, g.id] })}
                  className={`flex items-center gap-1 px-2 py-1 rounded-md text-[11px] border transition ${
                    checked ? "bg-primary/15 border-primary/50 text-primary glow-neon" : "bg-card border-border text-muted-foreground hover:border-primary/40"
                  }`}>
    ...
    content = perm && owner ? (
      <div className="space-y-3">
        <Block title="Permission node">
          <code className="block px-2.5 py-2 bg-input/40 rounded-md border border-border text-xs font-mono break-all">{perm.node}</code>
          {perm.description && <div className="text-[11px] text-muted-foreground mt-2">{perm.description}</div>}
        </Block>
        <Block title="Value">
          <ToggleRow label={perm.value ? "Allow" : "Deny"} checked={perm.value}
            onChange={(v) => updatePermission(selection.ownerType, selection.ownerId, perm.id, { value: v })} />
        </Block>
        <Block title="Temporary">
          <ToggleRow label="Expires after a while" icon={<Clock className="w-3 h-3" />} checked={!!perm.temporary}
            onChange={(v) => updatePermission(selection.ownerType, selection.ownerId, perm.id, { temporary: v, expiry: v ? Date.now() + 86400000 * 7 : undefined })} />
          {perm.temporary && (
            <Field label="Duration">
              <select className="w-full h-8 px-2 text-xs rounded-md bg-input border border-border focus:border-primary outline-none"
                value={String(perm.expiry || 0)}
                onChange={(e) => updatePermission(selection.ownerType, selection.ownerId, perm.id, { expiry: Number(e.target.value) })}>
                {[
                  ["1 hour", 3600e3], ["1 day", 86400e3], ["1 week", 7 * 86400e3], ["30 days", 30 * 86400e3], ["90 days", 90 * 86400e3],
                ].map(([l, ms]) => <option key={String(l)} value={Date.now() + Number(ms)}>{l}</option>)}
              </select>
            </Field>
          )}
        </Block>
        <Block title="Context">
          <Field label="Server">
            <select value={perm.server || ""} onChange={(e) => updatePermission(selection.ownerType, selection.ownerId, perm.id, { server: e.target.value })}
              className="w-full h-8 px-2 text-xs rounded-md bg-input border border-border focus:border-primary outline-none">
              <option value="">global</option>
              {["survival", "creative", "lobby", "minigames", "pvp"].map(s => <option key={s}>{s}</option>)}
            </select>
          </Field>
          <Field label="World">
            <select value={perm.world || ""} onChange={(e) => updatePermission(selection.ownerType, selection.ownerId, perm.id, { world: e.target.value })}
              className="w-full h-8 px-2 text-xs rounded-md bg-input border border-border focus:border-primary outline-none">
              <option value="">any world</option>
              {["world", "world_nether", "world_the_end", "spawn", "resource"].map(s => <option key={s}>{s}</option>)}
            </select>
          </Field>
        </Block>
    ...
    <aside className="w-80 shrink-0 bg-sidebar-bg border-l border-border flex flex-col">
      <div className="h-9 px-3 border-b border-border flex items-center gap-2 text-[11px] uppercase tracking-wider font-semibold text-muted-foreground">
        ...
        <span className="ml-auto flex items-center gap-1">
          {totalIssues > 0
            ? <span className="flex items-center gap-1 text-warning normal-case font-mono"><AlertTriangle className="w-3 h-3" />{totalIssues}</span>
            : <span className="flex items-center gap-1 text-success normal-case font-mono"><CheckCircle2 className="w-3 h-3" />clean</span>}
        </span>
      </div>
      <motion.div key={JSON.stringify(selection)} initial={{ opacity: 0, x: 10 }} animate={{ opacity: 1, x: 0 }}
...
const IssuesBlock = ({ issues }: any) => issues.length > 0 ? (
  <div className="rounded-lg border border-warning/30 bg-warning/5">
    ...
    <div className="p-2 space-y-1">
      {issues.map((i: any, idx: number) => (
        <div key={idx} className="text-[11px] px-2 py-1 rounded text-warning/90 bg-warning/5">{i.message}</div>
      ))}
    </div>
  </div>
) : null;
