...
import { Asterisk, Ban, Clock, Globe, Trash2, Layers, Sparkles, AlertTriangle, Info, ChevronUp, ChevronDown, X, Search } from "lucide-react";
  ...
  return (
    <motion.div layout
      ...
      className={`group flex items-center gap-2 px-3 py-1.5 rounded-md text-xs font-mono cursor-pointer border transition-all ${
        active ? "bg-primary/10 border-primary/60 glow-neon" :
        myIssues.length ? "border-warning/40 bg-warning/5" :
      ...
      {perm.plugin && <span className="text-[9px] px-1.5 py-0.5 rounded bg-secondary/60 text-muted-foreground normal-case font-sans shrink-0">{perm.plugin}</span>}
      {perm.temporary && <Clock className="w-3 h-3 text-info shrink-0" />}
      {perm.contexts && Object.keys(perm.contexts).length > 0 && <Globe className="w-3 h-3 text-info shrink-0" />}
      {myIssues.length > 0 && (
        <span title={myIssues.map((i: any) => i.message).join("\n")} className="shrink-0">
          <AlertTriangle className="w-3 h-3 text-warning" />
        </span>
      )}
      {inherited && <span className="ml-auto text-[9px] text-muted-foreground italic font-sans shrink-0">↳ {fromGroup}</span>}
      {!inherited && (
...
export function PermissionsPanel({ collapsed, onToggle }: { collapsed: boolean; onToggle: () => void }) {
  ...
  if (owner) {
    ...
    if (ownerType === "group") {
      ...
      inherited = Array.from(eff.entries()).filter(([n]) => !directSet.has(n)).map(([node, v]) => ({ node, value: v.value, from: v.from }));
      parentChips = owner.parents.map((pid: string) => groups.find(g => g.id === pid)).filter(Boolean);
    } else {
      // resolve via groups
      ...
      const directSet = new Set(direct.map(p => p.node));
      inherited = Array.from(merged.entries()).filter(([n]) => !directSet.has(n)).map(([node, v]) => ({ node, value: v.value, from: v.from }));
      parentChips = owner.groups.map((gid: string) => groups.find(g => g.id === gid)).filter(Boolean);
    }
  }
  ...
  const filteredDirect = direct.filter(p => !filter || p.node.toLowerCase().includes(filter.toLowerCase()));
  const filteredInh = inherited.filter(p => !filter || p.node.toLowerCase().includes(filter.toLowerCase()));
  ...
  if (collapsed) {
    return (
      <div className="border-t border-border bg-titlebar/80 glass">
        <button onClick={onToggle} className="w-full px-4 py-2 flex items-center gap-3 text-xs hover:bg-accent/30 transition">
          <ChevronUp className="w-4 h-4 text-primary" />
          <span className="text-muted-foreground">Permissions panel</span>
          {owner && <span className="font-mono text-primary">{title} · {direct.length} direct · {inherited.length} inherited</span>}
          {errCount > 0 && <span className="ml-auto text-[10px] px-1.5 py-0.5 rounded bg-destructive/20 text-destructive font-mono">{errCount} errors</span>}
          {warnCount > 0 && <span className="text-[10px] px-1.5 py-0.5 rounded bg-warning/20 text-warning font-mono">{warnCount} warnings</span>}
        </button>
      </div>
    );
  }
  ...
  return (
    <motion.div initial={{ height: 0 }} animate={{ height: 360 }} exit={{ height: 0 }}
      transition={{ type: "spring", stiffness: 200, damping: 26 }}
      className="border-t border-border bg-titlebar/90 glass flex flex-col overflow-hidden">
      <div className="px-4 py-2.5 border-b border-border flex items-center gap-3">
        <Sparkles className="w-4 h-4 text-primary" />
        <span className="text-sm font-semibold tracking-tight">{owner ? title : "No selection"}</span>
        {owner && (
            ...
            <div className="flex bg-input/40 border border-border rounded-md p-0.5 mr-2">
              {(["direct", "inherited", "issues"] as const).map(t => (
                <button key={t} onClick={() => setTab(t)}
                  ...
                  {t} {t === "direct" ? `(${direct.length})` : t === "inherited" ? `(${inherited.length})` : `(${ownerIssues.length})`}
                </button>
              ))}
            </div>
          )}
          {owner && <PermissionPicker ownerType={ownerType} ownerId={owner.id} existing={existing} />}
          <button onClick={onToggle} className="ml-1 p-1 hover:bg-accent rounded"><ChevronDown className="w-4 h-4" /></button>
        </div>
      </div>
      ...
      {owner && parentChips.length > 0 && (
        <div className="px-4 py-2 border-b border-border/50 flex items-center gap-2 flex-wrap">
          ...
          <span className="text-[10px] uppercase tracking-wider text-muted-foreground font-mono">{ownerType === "group" ? "inherits" : "in groups"}</span>
          {parentChips.map((p: any) => (
            <motion.button key={p.id} whileHover={{ scale: 1.05 }} onClick={() => setSelection({ type: "group", id: p.id })}
              ...
              {p.name}
              {ownerType === "user" && (
                <span onClick={(e) => { e.stopPropagation(); updateUser(owner.id, { groups: owner.groups.filter((g: string) => g !== p.id) }); }}
          ...
          <div className="space-y-1">
            <AnimatePresence mode="popLayout">
              {filteredDirect.length === 0 && (
                <div className="text-center text-xs text-muted-foreground py-6">No direct permissions yet — click <span className="text-primary font-semibold">+ Add permission</span> above</div>
              )}
              {filteredDirect.map(p => <PermRow key={p.id} perm={p} ownerType={ownerType} ownerId={owner.id} issues={ownerIssues} />)}
            </AnimatePresence>
          </div>
        )}
        {owner && tab === "inherited" && (
          <div className="space-y-1">
            {filteredInh.length === 0 && <div className="text-center text-xs text-muted-foreground py-6">No inherited permissions</div>}
            {filteredInh.map(p => <PermRow key={p.node} perm={{ id: p.node, node: p.node, value: p.value }} ownerType={ownerType} ownerId={owner.id} inherited fromGroup={p.from} />)}
          </div>
        )}
        {owner && tab === "issues" && (
          <div className="space-y-1.5">
            {ownerIssues.length === 0 && (
              <div className="text-center text-xs text-success py-6 flex flex-col items-center gap-2">
                <div className="w-10 h-10 rounded-full bg-success/15 grid place-items-center"><Sparkles className="w-5 h-5 text-success" /></div>
            ...
            {ownerIssues.map((i, idx) => (
              <motion.div key={idx} initial={{ opacity: 0, x: -8 }} animate={{ opacity: 1, x: 0 }}
                className={`flex items-start gap-2 px-3 py-2 rounded-md text-xs border ${
                  i.level === "error" ? "bg-destructive/10 border-destructive/30 text-destructive" :
                  i.level === "warning" ? "bg-warning/10 border-warning/30 text-warning" :
                  "bg-info/10 border-info/30 text-info"
                }`}>
                {i.level === "error" ? <AlertTriangle className="w-3.5 h-3.5 shrink-0 mt-0.5" /> :
                 i.level === "warning" ? <AlertTriangle className="w-3.5 h-3.5 shrink-0 mt-0.5" /> :
