...
export function PromptModalRoot() {
  ...
  useEffect(() => {
    openHandler = (o) => new Promise((resolve) => {
      setOpts(o); setValue(o.defaultValue || ""); setResolver(() => resolve);
    });
    return () => { openHandler = null; };
  }, []);

  const close = (v: string | null) => { resolver?.(v); setOpts(null); setResolver(null); };
