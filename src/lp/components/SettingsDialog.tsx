...
export function SettingsDialog({ open, onOpenChange, onTutorial }: { open: boolean; onOpenChange: (o: boolean) => void; onTutorial: () => void }) {
  const cfg = getConfig();
  ...
  const addPlugin = async () => {
    const name = await showPrompt({ title: "Plugin name", placeholder: "MyPlugin", submitLabel: "Next" });
    if (!name) return;
    const perms = await showPrompt({ title: `Permissions for ${name}`, placeholder: "myplugin.use, myplugin.admin", submitLabel: "Add", multiline: true });
    if (!perms) return;
    const list = perms.split(/[\n,]+/).map(s => s.trim()).filter(Boolean);
    const extra = {
      plugin: name, color: accent, version: "custom",
      categories: [{ name: "Custom", permissions: list.map(node => ({ node, default: "op" as const, wildcard: node.endsWith(".*") })) }],
    };
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-xl bg-panel border-border p-0 overflow-hidden">
        <DialogHeader className="px-5 py-3 border-b border-border bg-gradient-to-r from-titlebar to-panel relative">
          <div className="absolute inset-0 opacity-40 pointer-events-none" style={{ background: "var(--gradient-glint)" }} />
          <DialogTitle className="flex items-center gap-2 relative"><Settings className="w-4 h-4 text-primary" /> Settings</DialogTitle>
        </DialogHeader>
        <div className="p-5 space-y-4 max-h-[70vh] overflow-y-auto">
          ...
          <Section title="Custom plugins" icon={<Layers className="w-3.5 h-3.5" />}>
            <div className="space-y-1.5">
              {(cfg.extraPlugins||[]).map(p => (
                <div key={p.plugin} className="flex items-center gap-2 px-2 py-1.5 rounded-md bg-card border border-border">
                  ...
                  <span className="ml-auto text-[10px] font-mono text-muted-foreground">{p.categories.reduce((s,c)=>s+c.permissions.length,0)} perms</span>
                </div>
              ))}
              {!(cfg.extraPlugins||[]).length && <div className="text-[11px] text-muted-foreground italic">No custom plugins yet</div>}
              <button onClick={addPlugin} className="glint w-full h-8 flex items-center justify-center gap-1.5 rounded-md border border-dashed border-border hover:border-primary/50 hover:bg-primary/5 text-xs text-muted-foreground hover:text-primary transition">
          ...
          <Section title="Help" icon={<Sparkles className="w-3.5 h-3.5" />}>
            <button onClick={() => { onOpenChange(false); onTutorial(); }} className="glint w-full h-9 flex items-center justify-center gap-2 rounded-md border border-primary/40 bg-primary/5 hover:bg-primary/10 text-sm transition">
          ...
          <Section title="Danger zone" icon={<Trash2 className="w-3.5 h-3.5" />}>
            <div className="flex gap-2">
              <button onClick={() => { loadDemoData(); toast.success("Demo data restored"); }}
