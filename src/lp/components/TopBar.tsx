    ...
    <div className="h-11 border-b border-border bg-titlebar/80 glass flex items-center px-2 gap-1 select-none relative z-30">
      ...
      >
        <motion.div
          ...
          className="w-7 h-7 rounded-md grid place-items-center bg-primary/15 border border-primary/30 glow-green relative overflow-hidden"
        >
        ...
        <div className="leading-tight hidden sm:block max-w-[180px]">
          <div className="text-[13px] font-semibold tracking-tight truncate">
            ...
            <span className="text-primary">
              {brand.name.split(" ").slice(1).join(" ") || "Studio"}
            </span>
          </div>
          <div className="text-[9px] text-muted-foreground -mt-0.5 font-mono uppercase tracking-widest truncate">
            {brand.tagline}
          </div>
        </div>
      </motion.div>
      ...
      <button
        ...
        className="glint flex items-center gap-2 h-7 px-2.5 rounded-md bg-input/60 border border-border text-xs text-muted-foreground flex-1 max-w-72 min-w-0 hover:bg-input hover:border-primary/40 transition"
      >
        <Search className="w-3 h-3 shrink-0" />{" "}
        <span className="truncate">Search permissions, groups, users…</span>{" "}
        <span className="ml-auto font-mono text-[10px] opacity-60 hidden md:inline">⌘K</span>
      </button>
      <NavBtn
        icon={PlusCircle}
        label="New Group"
        onClick={() => addGroup(makeQuickName("group"))}
        hideLabelOn="lg"
      />
      <NavBtn
        icon={UserPlus}
        label="New User"
        onClick={() => addUser(makeQuickName("user"))}
        hideLabelOn="lg"
      />
...
function NavBtn({ icon: Icon, label, onClick, accent, hideLabelOn }: any) {
  const labelClass =
    hideLabelOn === "md" ? "hidden xl:inline" : hideLabelOn === "lg" ? "hidden 2xl:inline" : "";
  return (
    <motion.button
      whileHover={{ y: -1 }}
      whileTap={{ scale: 0.96 }}
      ...
      className={`glint flex items-center gap-1.5 h-7 px-2.5 rounded-md text-xs font-medium transition border ${
        accent
          ? "text-primary border-primary/30 bg-primary/5 hover:bg-primary/10 hover:border-primary/50"
