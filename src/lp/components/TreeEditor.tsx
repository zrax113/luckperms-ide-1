...
function PermissionRow({
  ...
  return (
    ...
    >
      ...
      {perm.plugin && (
        <span className="text-[9px] px-1.5 py-0.5 rounded bg-secondary/60 text-muted-foreground normal-case font-sans">
          {perm.plugin}
        </span>
      )}
      ...
      {inherited && (
        <span className="ml-auto text-[9px] text-muted-foreground italic font-sans">
          ↳ {fromGroup}
        </span>
      )}
      {!inherited && (
...
function AddPermission({ ownerType, ownerId }: { ownerType: "group" | "user"; ownerId: string }) {
  ...
  const matches = useMemo(
    () =>
      q.length < 1
        ? []
        : ALL_PERMISSIONS.filter((p) => p.node.toLowerCase().includes(q.toLowerCase())).slice(0, 8),
    [q],
  );
  if (!open)
    return (
      <button
        ...
        className="mt-2 flex items-center gap-1.5 px-2.5 py-1.5 text-xs rounded-md text-muted-foreground hover:text-primary hover:bg-primary/10 transition border border-dashed border-border hover:border-primary/40"
      >
    ...
    <div className="mt-2 rounded-md bg-card border border-primary/40 p-2 glow-primary">
      <div className="flex gap-2">
        <input
          ...
          onKeyDown={(e) => {
            if (e.key === "Enter" && q) {
              addPermission(ownerType, ownerId, q);
              setQ("");
              setOpen(false);
            }
        ...
        <div className="mt-2 space-y-0.5 max-h-48 overflow-y-auto">
          {matches.map((m) => (
            <button
              key={m.node}
              onClick={() => {
                addPermission(ownerType, ownerId, m.node, m.plugin);
                setOpen(false);
                setQ("");
              }}
            ...
            >
              {m.node.endsWith(".*") ? (
                <Asterisk className="w-3 h-3 text-warning" />
              ) : (
                <Sparkles className="w-3 h-3 text-info" />
              )}
              <span className="truncate">{m.node}</span>
              <span className="ml-auto text-[9px] px-1.5 py-0.5 rounded bg-secondary/60 normal-case font-sans">
                {m.plugin}
              </span>
            </button>
          ))}
...
export function TreeEditor() {
        ...
        <div className="p-4 space-y-4 max-w-4xl">
          ...
          <Section title="Direct Permissions" icon={Sparkles} count={u.permissions.length}>
            <div className="space-y-1">
              {u.permissions.map((p) => (
                <PermissionRow key={p.id} perm={p} ownerType="user" ownerId={u.id} />
              ))}
  ...
  return (
    <div className="flex-1 overflow-y-auto">
      <Header
        title={g.name}
        subtitle={`weight ${g.weight} • ${g.parents.length} parent${g.parents.length === 1 ? "" : "s"}`}
        icon="group"
        color={g.color}
      />
      <div className="p-4 space-y-4 max-w-4xl">
        <Section title="Inheritance Chain" icon={Layers}>
          {g.parents.length === 0 ? (
            <div className="text-xs text-muted-foreground italic">No parent groups</div>
          ) : (
              ...
              {g.parents.map((pid) => {
                ...
                return (
                  <motion.div
                    ...
                    className="flex items-center gap-2 px-3 py-1.5 rounded-md bg-card border border-border text-xs glint"
                  >
                    <div
                      className="w-2 h-2 rounded-full"
        ...
        <Section title="Direct Permissions" icon={Sparkles} count={g.permissions.length}>
          <div className="space-y-1">
            <AnimatePresence>
              {g.permissions.map((p) => (
                <PermissionRow key={p.id} perm={p} ownerType="group" ownerId={g.id} />
              ))}
            </AnimatePresence>
  ...
  return (
    <div className="rounded-lg border border-border bg-panel/50 backdrop-blur-sm">
      <div className="flex items-center gap-2 px-3 py-2 border-b border-border/60">
        ...
        {count !== undefined && (
          <span className="text-[10px] bg-muted px-1.5 py-0.5 rounded font-mono">{count}</span>
        )}
      </div>
    ...
    <div className="flex-1 grid place-items-center text-center p-8">
      <div className="max-w-sm">
        ...
        <p className="text-sm text-muted-foreground mt-2">
          Pick something from the explorer to start editing permissions visually. Drag, drop, and
          inherit — no YAML required.
        </p>
      </div>
    </div>
  );
